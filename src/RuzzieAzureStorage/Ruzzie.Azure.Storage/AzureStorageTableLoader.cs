...
namespace Ruzzie.Azure.Storage
{
    ...
    public class AzureStorageTableLoader<TIn, TOut> : IDisposable where TIn : ITableEntity, new()
    {
        ...
        private async Task<ReadOnlyCollection<TOut>> ReadAllEntitiesFromTableStorage(
            IEnumerable<string> allPartitionKeys)
        {
            var res = Task.Run(async () =>
                               {
                                   ...
                                   Parallel.ForEach(allPartitionKeys
                                                  , partitionKey =>
                                                    {
                                                        allTasks.Add(ReadAllEntitiesForPartitionKey(partitionKey
                                                                                                  , sharedItemList));
                                                    });
